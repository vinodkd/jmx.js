...
function displayNode(node, displayLoc){
	...
	if(config) {
		createView();
		addInteractions();
		if(config.processChildren || shouldProcessChildren()){
			processChildren();
		}
	}
...
function toggleAttrs (e) {
	expandOrCollapse(e.srcElement.parentElement.nextElementSibling);
}

function toggleChildren (e) {
	expandOrCollapse(e.srcElement.nextElementSibling.nextElementSibling);
	e.srcElement.innerHTML = (e.srcElement.innerHTML.trim() == "^" ? " > " : " ^ ");
}
