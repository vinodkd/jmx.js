...
var ELEMENTS = {
	"jmeterTestPlan" 	: { processChildren: true },
	"hashTree"			: { processChildren: true },
	"TestPlan"			: { processChildren: true },
	"ThreadGroup" : {
		childElements	: {},
		attrs 			: {
	...
	"ConfigTestElement"	: { processChildren: true },
	"GenericController"	: { processChildren: true },
	"HTTPSampler"		: { processChildren: true },
	"GenericController"	: { processChildren: true },
	"ResultCollector"	: { processChildren: true },
	"elementProp"		: { processChildren: true },
	"objectProp"		: { processChildren: true },
	"collectionProp"	: { processChildren: true },
	"boolProp"			: {
							attrs: {
								name 	: { path : "@name" },
								value 	: { path : "." }
							},
							view: '<div> <span class="label"><%=name%></span><input type="checkbox" name="<%=name%>" value="<%=value%>" <%=(value=="true" ? "checked" : "")%> ></input> </div>',
							'type' : 'boolean'
						  },
	"stringProp"		: {
							attrs: {
							...
							view: '<div> <span class="label"> <%=name%> </span><input type="textbox" name="<%=name%>" value="<%=value%>"></input> </div>' 
						  },
	"longProp"			: // long is a string for display purposes and to save to file. validation tbd
						  {
						    ...
						    view: '<div> <span class="label"> <%=name%> </span><input type="textbox" name="<%=name%>" value="<%=value%>" bleh></input> </div>',
						    'type' : 'number'
						  },
	"DEFAULT"			: {
							attrs: {
							...
							view: '<div> No handler for the node named <%=name%>. </div>' 
						  },
...
function loadTemplates () {
	for (var el in ELEMENTS) {
		...
		if(element.attrs){
			if(element.view){ loadTemplate(el,element.view);}
			else{ loadTemplate(el); }
			;	
		}
...
function loadTemplate (tref, view) {
	var template;
	if(view){
		template = view;
	}
	else{
		var asynchttp = loadFile("res/"+tref+".tmpl");
...
function displayNode(node, displayLoc){
	...
	var elementKey = (ELEMENTS[node.nodeName]) ? node.nodeName : "DEFAULT";
